...
@SuppressFBWarnings(
        ...
        justification = "Part of public API, reviewed all uses of Plugin are fully quantified in this class")
public class Plugin extends hudson.Plugin {
    ...
    public static final class RenameListener extends ItemListener {
            ...
            for (Project<?, ?> p : Jenkins.get().getAllItems(Project.class)) {
                ...
                if (changed) {
                    try {
                        ...
                        Logger.getLogger(RenameListener.class.getName())
                                .log(
                                        Level.WARNING,
                                        "Failed to persist project setting during rename from " + fullOldName + " to "
                                                + fullNewName,
                                        e);
                    }
                }
            ...
            for (Project<?, ?> p : Jenkins.get().getAllItems(Project.class)) {
                ...
                if (changed) {
                    try {
                        ...
                        Logger.getLogger(RenameListener.class.getName())
                                .log(Level.WARNING, "Failed to persist project setting during remove of " + oldName, e);
                    }
                }
            }
        }
        ...
        private boolean renameInConditionalBuildStep(Project<?, ?> p, String oldName, String newName) {
            boolean changed = false;
            final List<TriggerBuilder> containedBuilders =
                    ConditionalBuildStepHelper.getContainedBuilders(p, TriggerBuilder.class);
        ...
        private boolean deleteInConditionalBuildStep(Project<?, ?> p, String oldName) {
            ...
            for (TriggerBuilder triggerBuilder : containedBuilders) {
                for (ListIterator<BlockableBuildTriggerConfig> bbtc =
                                triggerBuilder.getConfigs().listIterator();
                        bbtc.hasNext(); ) {
                    BuildTriggerConfig c = bbtc.next();
                    if (c.onDeleted(p.getParent(), oldName)) {
                        changed = true;
